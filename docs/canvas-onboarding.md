...
The guide offers Text to image, Script to speech, Text to video, and Image to video. Availability comes from the current generation configuration; unavailable examples explain which capability is missing. Examples use supported inputs and describe current limits: image references and video-to-video are unsupported, Text generation consumes only text context, and Speech → Video Audio is composition rather than a video-model input.
