...
For example, select an older **Coffee scene** image, then choose **Run to this node** on **Coffee video**. The review includes only video generation and uses the selected image without an image-generation charge. Pinned text similarly supplies its exact saved output without regenerating its upstream branch. If the pinned source is explicitly selected as a workflow output too, that source generates a new attempt, while its consumer still uses the pinned result. An independently connected, unpinned prompt branch runs normally.
