...
## Product-photo reference editing

- September 20, 2026, working tree based on `c790602`: **502 application tests passed** (generation 267, projects 63, media 51, API 85, email 18, billing 18). New integration checks execute migration `0027_image_references` in isolated Postgres and cover frozen private reference bytes, historical selection, generated-image chains, permissions lost before execution, missing storage, cross-project rejection, single charging across retries, and selective reuse after reference changes.
- All **10 workspace type-check tasks** passed. The final web check passed after updating node labels. Changed code passes Biome and `git diff --check`. The jobs Worker dry-run bundle passed. No public deployment was made.
- Applied `0027_image_references` to the database used by the running development app. Existing no-reference image hashes remain stable so old workflow requests can still resume. No new keys or public image-delivery origin are needed.
- Created **Product photo editing verification**, project/canvas `69379c02-a56d-4441-9c2c-d825450f9a68`, through the in-app browser. Added **Guide → Edit a product photo**, uploaded a synthetic 512 × 512 bottle fixture, and ran one real reference edit through FLUX.2 [klein] 4B. The saved output is a 1024 × 1024 JPEG, asset `13793aec-cfaf-4a85-97b8-e89040bba97d`; reference asset `8d9e7d4b-e182-4b36-9e9d-71bae13c0e36` remains available. Visual inspection showed the bottle and label mark retained in a new beige studio composition. This single fixture does not establish exact brand/logo fidelity for arbitrary photos.
- The balance changed once from **421 to 418 Kousa credits**. Reload preserved the reference, connection, generated output, and **Up to date** state. **Run affected steps** reviewed one reused image result at **0 credits**, without regenerating the source. The preview was closed without starting another run. Final Next.js runtime and compilation diagnostics were clean.
- This first feature supports one reference and natural-language edits. Masks, brush editing, multiple references, editable text/logo overlays, and batch variations remain separate work.
