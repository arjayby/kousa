...
Text nodes use the Vercel AI Gateway through AI SDK. Set `AI_GATEWAY_API_KEY` in `apps/web/.env`, then restart `pnpm dev`. Alchemy passes the secret to the web and generation Workers. The generation Worker runs Cloudflare Workflows; no extra key or hosting provider is required. See [background generation](background-generation.md).
...
The `kousa_claim_generation` Postgres function introduced in migration `0006_text_generation.sql` and extended in `0008_image_generation.sql` and `0009_durable_generation.sql` locks the payer and project, rechecks editor access and balance, and inserts a reservation in one transaction. This works with Neon's HTTP driver. **Use migrations, not `db:push` alone:** Drizzle's schema does not represent this function.

Available balance is purchase grants minus successful runs and unexpired reservations. Output and the final charge are committed in one transaction that rechecks editor access. Failure releases the reservation. A thirty-minute deadline bounds queued and running reservations. A scheduled sweep, project read or new claim marks expired records failed. Late completions cannot charge an expired run.
...
Canvas tabs poll server-owned results every three seconds while active. This keeps generation state independent of Liveblocks and the future Synixir migration. The request returns after queuing; generation continues in a separate Worker. Saved input snapshots and private R2 response receipts support durable recovery. An ambiguous interrupted provider call is never automatically repeated; it fails and releases the reservation if no saved response can be recovered. Bounded text-to-image generation is documented in [Image generation](image-generation.md).
