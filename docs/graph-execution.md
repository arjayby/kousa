...
An image selected through **Generation history** behaves as a fixed starting image too. Its exact run and asset are frozen into the plan. Selected historical text supplies its saved output without regenerating its upstream branch. Selecting the source node as an explicit output still generates it, while consumers retain their selected historical input. Missing selections fail before reservation. See [node generation history](node-generation-history.md).
