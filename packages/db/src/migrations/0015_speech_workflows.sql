-- Preserve graph reservations and video inputs while copying the saved speech
-- voice and delivery direction into each queued child generation.
...
BEGIN
  ...
  END IF;
  INSERT INTO generation_run(id, graph_run_id, project_id, node_id, user_id, model_id, prompt, input_hash, status, credits, expires_at, kind, size, stage, duration, aspect_ratio, input_image_asset_id, input_image_origin, voice_id, voice_direction)
    VALUES(child, flow.id, flow.project_id, (planned_step->>'nodeId')::uuid, flow.user_id, planned_step->>'modelId', p_prompt, planned_step->>'inputHash', 'queued', cost,
      ...
      (planned_step->>'duration')::integer, planned_step->>'aspectRatio', input_image, input_origin, planned_step->>'voiceId', planned_step->>'voiceDirection');
