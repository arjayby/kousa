-- Validate frozen media under the same project lock as credit reservation.
CREATE OR REPLACE FUNCTION kousa_valid_generation_media(inputs jsonb, project_id uuid) RETURNS boolean LANGUAGE sql STABLE AS $$
  SELECT jsonb_array_length(coalesce(inputs->'media', '[]'::jsonb)) <= 8
    AND NOT EXISTS (
      SELECT 1 FROM jsonb_array_elements(coalesce(inputs->'media', '[]'::jsonb)) item
      WHERE NOT EXISTS (
        SELECT 1 FROM media_asset a WHERE a.id = (item->>'assetId')::uuid
          AND a.project_id = $2 AND a.status = 'ready'
          AND ((item->>'output' IN ('lastFrame', 'audio') AND a.mime_type = 'video/mp4')
            OR (item->>'output' IS NULL AND item->>'kind' = 'image' AND a.mime_type IN ('image/png','image/jpeg','image/webp'))
            OR (item->>'output' IS NULL AND item->>'kind' = 'audio' AND a.mime_type = 'audio/mpeg')
            OR (item->>'output' IS NULL AND item->>'kind' = 'video' AND a.mime_type = 'video/mp4'))
      )
    );
$$;
...
ALTER TABLE generation_run ADD CONSTRAINT generation_input_image_valid CHECK (
  ...
  OR (kind = 'video' AND (input_image_asset_id IS NOT NULL OR jsonb_array_length(coalesce(resolved_inputs->'media', '[]'::jsonb)) > 0) AND input_image_origin IS NOT NULL
...
BEGIN
  ...
  IF (p_input_image IS NULL AND p_input_origin IS NOT NULL AND NOT (p_kind = 'video' AND jsonb_array_length(coalesce(p_resolved_inputs->'media', '[]'::jsonb)) > 0))
  ...
  IF NOT kousa_valid_generation_media(p_resolved_inputs, p_project) THEN RETURN '{"claimed":false,"error":"CONFLICT"}'::jsonb; END IF;
...
BEGIN
  ...
  END IF;
  IF NOT kousa_valid_generation_media(p_resolved_inputs, flow.project_id) THEN RETURN false; END IF;
  IF planned_step->>'kind' = 'video' AND jsonb_array_length(coalesce(p_resolved_inputs->'media', '[]'::jsonb)) > 0 THEN
    input_origin := nullif(planned_step->>'inputImageOrigin', '');
    IF input_origin IS NULL THEN RETURN false; END IF;
  END IF;
