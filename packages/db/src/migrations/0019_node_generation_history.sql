ALTER TABLE "generation_run" ADD COLUMN "authored_settings" jsonb;
--> statement-breakpoint
DROP FUNCTION kousa_claim_generation(uuid, text, uuid, uuid, text, text, text, integer, text, text, text, text, integer, text, uuid, text);
--> statement-breakpoint
CREATE FUNCTION kousa_claim_generation(
  ...
  p_input_image uuid DEFAULT NULL, p_input_origin text DEFAULT NULL, p_authored_settings jsonb DEFAULT NULL
...
BEGIN
  ...
  IF (result->>'claimed')::boolean THEN
    UPDATE generation_run SET input_image_asset_id = p_input_image, input_image_origin = p_input_origin, authored_settings = p_authored_settings WHERE id = p_id;
  END IF;
  RETURN result;
END;
$$;
...
BEGIN
  ...
  END IF;
  INSERT INTO generation_run(id, graph_run_id, project_id, node_id, user_id, model_id, prompt, input_hash, status, credits, expires_at, kind, size, stage, duration, aspect_ratio, input_image_asset_id, input_image_origin, voice_id, voice_direction, authored_settings)
    VALUES(child, flow.id, flow.project_id, (planned_step->>'nodeId')::uuid, flow.user_id, planned_step->>'modelId', p_prompt, planned_step->>'inputHash', 'queued', cost,
      ...
      (planned_step->>'duration')::integer, planned_step->>'aspectRatio', input_image, input_origin, planned_step->>'voiceId', planned_step->>'voiceDirection', planned_step->'authoredSettings');
